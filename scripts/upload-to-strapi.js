...
async function run() {
  ...
  const mdPath = path.join(projectDir, 'what-tool-is-this-identification-guide.md');
  ...
  // Parse content starting from line 7 onwards (after metadata lines)
  const bodyContent = lines.slice(6).join('\n').trim();
  ...
  const slug = 'what-tool-is-this-identification-guide';
  const excerpt = 'Struggling to identify an unknown or mystery tool? Learn the 5 most reliable ways to identify any tool instantly, from maker\'s marks to AI-powered computer vision.';
  const date = '2026-06-12';
  const author = 'ToolScan Team';
  const keywords = ['tool identification', 'identify tools', 'mystery tools', 'ToolScan app', 'workshop safety'];
  ...
  const payload = {
    data: {
      ...
      slug,
      excerpt,
      content: blocksContent, // Sending blocks JSON array instead of markdown string
      date,
      author,
      keywords,
      relatedPostIds,
